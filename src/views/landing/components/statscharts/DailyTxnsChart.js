...
`;
const FlexBoxLight = styled(FlexBox)`
  border: none;
  box-shadow: none;
`;
...
const ConsensusState = () => {
  const { totalTxsData, totalTxsDataLoading } = useSelector(
    (state) => state.graph
  );

  const arr = [];
  totalTxsData &&
    totalTxsData.graphData?.map((item) =>
      arr.push({
        price: item.totalTrx,
        time: item.time
      })
  ...
  const arr2 = [];
  arr1.map((item) =>
    arr2.push({ Price: item.price, Time: dayjs(item.time).format('DD MMM') })
  );
  arr2.reverse();
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <TooltipWrapper>
          <TooltipHead>Daily Txns</TooltipHead>
          <HorizontalLine />
          <TooltipContent>
            <TooltipLabel>
              <YellowIcon />
  ...
  return (
    <Wrapper loading={+totalTxsDataLoading}>
      <HeadingBox>
        <HeadingRow>
          <ChartsHeading
            title="Daily Txns"
          ...
          />
          <CoinTags>
            {/* work inprogress will be used later👇 */}
            {/* <FlexBoxLight>
              <RedIcon />
